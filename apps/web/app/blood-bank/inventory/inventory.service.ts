...
export class InventoryService {
  ...
  async discardUnit(principal: Principal, dto: DiscardUnitDto) {
    ...
    const unitKey = String((dto as any).unitId ?? (dto as any).bloodUnitId ?? "").trim();
    if (!unitKey) throw new BadRequestException("bloodUnitId is required");

    const unit =
      (await this.ctx.prisma.bloodUnit.findUnique({ where: { id: unitKey } })) ??
      (await this.ctx.prisma.bloodUnit.findFirst({ where: { branchId: bid, unitNumber: unitKey } }));
    if (!unit) throw new NotFoundException("Blood unit not found");
    this.ctx.resolveBranchId(principal, unit.branchId);
    ...
    await this.ctx.audit.log({
      ...
      action: "BB_UNIT_DISCARDED", entity: "BloodUnit", entityId: unit.id,
