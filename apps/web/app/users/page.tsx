...
function roleCodeLabel(roleCode: string, branchName: string | null) {
  const rc = String(roleCode || "").trim();
  if (!rc) return "-";
  if (rc === "SUPER_ADMIN" || rc === "CORPORATE_ADMIN") return "Corporate";
...
export default function SuperadminUsersPage() {
  ...
  const canCreate = perms ? perms.includes("IAM_USER_CREATE") : (isSuperAdmin || roleCode === "CORPORATE_ADMIN");
  const canUpdate = perms ? perms.includes("IAM_USER_UPDATE") : (isSuperAdmin || roleCode === "CORPORATE_ADMIN");
  const canReset = perms ? perms.includes("IAM_USER_RESET_PASSWORD") : (isSuperAdmin || roleCode === "CORPORATE_ADMIN");
      ...
      <div className="grid gap-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1 text-sm text-zc-muted">Corporate view across branches. Create users, assign roles, and manage access.</div>
            </div>
          </div>
        ...
        <Card className="overflow-hidden">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-sm">Search and manage users across all branches. BRANCH-scoped roles require selecting a branch.</CardDescription>
          </CardHeader>
              ...
              <div className="flex w-full flex-col gap-3 lg:flex-row lg:items-center">
                ...
                <div className="w-full lg:w-[320px]">
                  ...
                  >
                    ...
                    <option value="CORPORATE">Corporate (no branch)</option>
                    {branches.map((b) => (
          ...
          <CardHeader className="pb-4">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <CardDescription className="text-sm">Corporate user list (latest entries). Use search + branch filter.</CardDescription>
              </div>
              <div className="text-xs text-zc-muted">Last refresh updates list + role metadata</div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="overflow-auto rounded-xl border border-zc-border">
              <table className="w-full min-w-[980px] text-sm">
                <thead className="bg-zc-panel/40 text-zc-muted">
                  <tr>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-1 text-sm text-zc-muted">
                For BRANCH roles, select the target branch during creation. For corporate roles, keep branch empty.
              </div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="grid gap-5">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setForm((s) => ({
                    ...s,
                    roleCode: next,
                    // If switching to GLOBAL role, clear branch.
            ...
            <div className="grid gap-2">
              ...
              >
                <option value="">{needsBranch ? "Select branch (required)" : "Branch not required for GLOBAL roles"}</option>
                {branches.map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.name} ({b.code})
                  </option>
              ...
              <p className="text-[11px] text-zc-muted">{needsBranch ? "Required for BRANCH-scoped roles." : "Only BRANCH roles can have a branch."}</p>
            </div>
          </div>
        ...
        <DialogContent className="w-[95vw] sm:max-w-[640px] max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-5">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label>Full Name</Label>
                <Input value={detailsForm.name} onChange={(e) => setDetailsForm((s) => ({ ...s, name: e.target.value }))} disabled={!canUpdate} />
              </div>
            ...
            <div className="grid gap-2">
              ...
              <select
                ...
                disabled={!canUpdate || !detailsNeedsBranch}
              >
                <option value="">{detailsNeedsBranch ? "Select branch (required)" : "Branch not required for GLOBAL roles"}</option>
                {branches.map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.name} ({b.code})
                  </option>
              ...
              <p className="text-[11px] text-zc-muted">{detailsNeedsBranch ? "Required for BRANCH-scoped roles." : "Only BRANCH roles can have a branch."}</p>
            </div>

            {!canUpdate ? (
              <div className="rounded-xl border border-zc-border bg-zc-panel/20 p-3 text-xs text-zc-muted">You don't have permission to edit user details.</div>
            ) : null}
          </div>
